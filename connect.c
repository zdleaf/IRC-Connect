...
void sendping(int *socket, int size, char *buf){
	int bytes_sent, i;
	char pingbuf[size];
...
int proc_buffer(char *buffer, int *socket){
	...
	* note: the below string comparison else-if statements are temporary.
	**/
	...
	if ((token = strtok(buffer, delimiter)) != NULL) {				// split everything into buffers delimited by \n characters
		do {
			...
			if (strncmp(token, noticeauth, strlen(noticeauth)) == 0){	// if a notice auth, print it
				...
				sendping((int *)socket, strlen(token), token);
...
void proc_overflow(char *buffer, char *temp){
	...
	* the buffer can then be processed via proc_buffer, and recv2(ovrflag=1) can recv() on top of overflow
	**/
...
int recv2(int *socket, char *buffer, int *ovrflag, char *temp, int temp_len){
	
	/**
	* recv2()
	* recieves data from socket into malloc'd buffer.
	* if there's been an overflow, recv2() gets overflowed string from temp (from proc_overflow()) and concatenates it w/ new recv.
...
int main(int argc, char *argv[]){
	...
	while (recv2(&sockfd, buffer, &ovrflag, temp, strlen(temp)) != 0){
		...
		proc_buffer(buffer, &sockfd);		// process the buffer	
		for(i = 0; i <= MAXDATASIZE - 1; i++) {	// zero the buffer ready for new data
			buffer[i]='\0';
		}
	} 
	printf("Error in recv2()\n");	
}
